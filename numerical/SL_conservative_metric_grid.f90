program SL_conservative_metric_grid
    !====================================================================
    ! Sturm-Liouville eigenvalue solver for the Klein-Gordon equation
    ! on a static, spherically symmetric spacetime.
    !
    ! Spatial metric: gamma_rr(r) = 1 + 1/r  (Rs = 1 units)
    !
    ! The SL problem per multipole ell is
    !
    !   -d/dr[ p(r) dR/dr ] + Q_ell(r) R = k^2 w(r) R,
    !
    ! with
    !   p(r)       = r^2 / sqrt(gamma_rr),
    !   w(r)       = r^2 * sqrt(gamma_rr),
    !   Q_ell(r)   = ell*(ell+1) * sqrt(gamma_rr).
    !
    ! The last expression is the volume-weighted angular term:
    !   w(r) * ell*(ell+1)/r^2 = ell*(ell+1)*sqrt(gamma_rr).
    !
    ! Boundary conditions:
    !   left  : ell=0 -> Neumann R'=0; ell>0 -> Dirichlet R=0,
    !   right : Neumann R'=0 at the numerical outer boundary.
    !
    ! Discretization:
    !   Finite-volume generalized eigenproblem
    !       A R = k^2 M R,
    !   where M_i = w(r_i) * Delta r_c(i).
    !
    ! Symmetric form:
    !       B z = k^2 z,
    !       B = M^{-1/2} A M^{-1/2},
    !       R = M^{-1/2} z.
    !
    ! Grid and spectral-saving policy in this version:
    !   - r_target is NOT forced to be an exact node.
    !   - The grid keeps stepping with the natural local spacing until
    !     r_outer >= r_target.
    !   - Therefore r_outer may be slightly larger than r_target.
    !   - Modes are saved dynamically for each ell using a safe Nyquist cut:
    !         k <= k_safe = nyquist_safety*pi/dr_max.
    !   - n_modes_cap is only an upper cap, not a fixed number saved per ell.
    !====================================================================
    use hdf5
    implicit none
    ...
    real(kind=8), parameter :: dr_max   =     2.0d0
    real(kind=8), parameter :: r_flat   =  1000.0d0

    integer,      parameter :: ell_max     = 5
    ...
    real(kind=8), parameter :: k_safe = (2.0d0/dr_max) * &
                                        dsin(0.5d0*q_safe*dr_max)

    

    character(len=*), parameter :: h5_file = &
        "sl_spectrum_rtarget_10000_Ggrid_drmax2_rflat1000.h5"
    ! -------------------------------------------------------------------

    integer :: Nr, N_intervals, n_save_ell
    integer :: ell, i, j, lwork, info
    ...
    !====================================================================
    ! 1. Build grid and mass matrix
    !====================================================================
    call count_grid(r_target, N_intervals, r_outer)

    Nr = N_intervals - 1

    call print_grid_info(Nr, N_intervals, r_target, r_outer)

    allocate(r(0:Nr+1), dr_edge(0:Nr), mdiag(Nr))
    call build_grid(Nr, r, dr_edge)

    ! Recompute actual r_outer from the built grid.
    r_outer = r(Nr+1)

    ! Mass matrix: M_i = w(r_i) * Delta r_c(i), Delta r_c = cell width.
    do i = 1, Nr
    ...
    !====================================================================
    ! 2. Allocate eigenproblem workspace
    !====================================================================
    ...
    !====================================================================
    ! 3. Open HDF5 output file and store grid + run parameters
    !====================================================================
    ...
    call write_real_1d(grp_id, "real_extra", [r_outer, nyquist_safety, k_safe])
    call write_int_1d (grp_id, "integer",    [Nr, N_intervals, ell_max, n_modes_cap, -1])
    call h5gclose_f(grp_id, hdferr)

    !====================================================================
    ! 4. Solve for each ell
    !====================================================================
    do ell = 0, ell_max

        call build_B_matrix(Nr, ell, r, dr_edge, mdiag, D, E)

        ! LAPACK: D receives eigenvalues lambda = k^2, Z eigenvectors z.
        call dstev('V', Nr, D, E, Z, Nr, work, info)
        if (info /= 0) then
            write(*,'(A,I4,A,I8)') 'ERROR: dstev failed for ell=', ell, ', info=', info
            stop
        end if

        ! Determine how many eigenvalues are inside the safe Nyquist band.
        n_save_ell = 0
        do j = 1, Nr
            if (D(j) < -lambda_tol) then
                write(*,'(A,I4,A,I8,A,ES24.16)') &
        ...
        ! Recover physical modes R = M^{-1/2} z only for saved modes.
        do j = 1, n_save_ell
            do i = 1, Nr
        ...
        ! Explicit M-normalization guard for saved modes.
        do j = 1, n_save_ell
            norm = 0.0d0
            do i = 1, Nr
        ...
        call write_real_1d(grp_id, "k",     k_save(1:n_save_ell))
        call write_real_2d(grp_id, "modes", Z(:,1:n_save_ell))
        call write_int_scalar(grp_id, "n_save", n_save_ell)
        call write_real_1d(grp_id, "spectral_cut", [nyquist_safety, k_safe])
        call h5gclose_f(grp_id, hdferr)

        write(*,'(A,I4,A,I8,A,ES12.4,A,ES12.4,A,ES12.4)') &
    ...
    write(*,'(A,A)') 'Done. Wrote ', trim(h5_file)
...
contains
    ...
    subroutine build_B_matrix(Nr, ell, r, dr_edge, mdiag, D, E)
        ...
        do i = 1, Nr
            ...
            ! Left BC:
            ! ell=0 -> Neumann R_0=R_1, remove left flux.
            ! ell>0 -> Dirichlet R_0=0, keep left contribution.
            if (ell == 0 .and. i == 1) A_ii = A_ii - p_l/dr_edge(0)

            ! Right BC: Neumann R_{Nr+1}=R_Nr, remove right flux.
    ...
    ! Diagnostics
    !=================================================================
    subroutine print_grid_info(Nr, N_intervals, r_target, r_outer)
        integer,      intent(in) :: Nr, N_intervals
        real(kind=8), intent(in) :: r_target, r_outer

        write(*,'(A)')          'SL grid parameters:'
...
end program SL_conservative_metric_grid